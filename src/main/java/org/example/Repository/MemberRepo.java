...
public class MemberRepo {
    ...
    public boolean updateMember(int id, String name, String contact) {
        Transaction tx = null;
        try (Session session = DB.getSessionFactory().openSession()) {
            MemberModel member = session.get(MemberModel.class, id);
            if (member == null) {
    ...
    public List<MemberModel> getAllMembers() {
        try (Session session = DB.getSessionFactory().openSession()) {
            return session.createQuery("FROM MemberModel", MemberModel.class).list();
        } catch (Exception e) {
    ...
    public long getActiveBorrowCount(int memberId) {
        try (Session session = DB.getSessionFactory().openSession()) {
            Query<Long> query = session.createQuery(
                    "SELECT COUNT(bs) FROM BorrowSlipsModel bs " +
                            "WHERE bs.memberId = :memberId AND bs.status != :returned",
                    Long.class);
    ...
    public double getUnpaidFineTotal(int memberId) {
        try (Session session = DB.getSessionFactory().openSession()) {
            Query<Double> query = session.createQuery(
                    "SELECT COALESCE(SUM(f.amount), 0.0) FROM FinesModel f " +
                            "WHERE f.memberId = :memberId AND f.paid = false",
                    Double.class);
            query.setParameter("memberId", memberId);
